...
| **研究规划** | 用自然语言输入研究问题，系统自动匹配变量、模型和变换建议 |
| **因果语言检测** | 检测"导致/影响/因果"等表述，重新框定为关联分析并明确提示 |
| **吸收变量透明化** | 双向固定效应报告哪些变量被吸收，并警告主解释变量是否受影响 |
| **探索性发现** | 有界规范搜索、变量筛选、多重检验校正（BH/Bonferroni）、稳定性评估 |
| **多维评分** | 6 维加权评分：校正后统计支持、稳定性、方向一致性、数据质量、诊断质量、拟合优度 |
| **发现→规划衔接** | 一键将探索性发现转化为理论驱动的研究计划 |
| **持久化研究工作区** | 基于 SQLite 的项目管理，服务重启数据不丢失——将数据集、分析和报告组织在同一项目下 |
| **时间线追踪** | 项目活动按时间顺序记录——数据上传、分析、比较、报告 |
| **归档历史** | 浏览项目下所有关联的归档物（数据集、分析、计划、报告、发现） |
| **可复现打包导出** | 下载项目 ZIP 包——元数据、归档物、时间线，可选包含原始数据 |
| **重启恢复** | DataFrame 内存缓存，重启后从磁盘惰性加载——数据零丢失 |
| **出版级回归表** | 学术回归表（单模型、多模型比较、系数稳定性、描述统计、诊断汇总），含 *** 显著性标记 |
| **学术图表** | 系数图、残差与拟合值图、实际与预测图、相关矩阵热图、稳定性图——全部使用 matplotlib |
| **DOCX 导出** | 可真正编辑的 Word 文档，含封面、表格、图表和专业排版（python-docx） |
| **LaTeX 导出** | 独立 .tex 文件 + tables/ + figures/ + appendix/ 目录——可直接用 pdflatex 编译 |
| **方法论附录** | 自动生成方法论章节：变量选择、模型设定、研究局限 |
| **可复现附录** | 数据集校验和、变换日志、模型配置、软件版本——全部确定性生成 |
...
上传 → 剖析 → 变量配置 → 数据变换 → 模型执行 → 结果展示
              ↘ 探索发现 → 变量筛选 → 规范搜索 → 多重校正 → 发现排名 → 研究规划
```

```
上传 → 剖析 → 变量配置 → 数据变换 → 模型执行 → 结果展示
     ↘ 项目工作区 → 数据集 → 分析 → 时间线 → 打包导出
```
...
├── backend/
│   ├── app/
│   │   ├── analysis/          # diagnostics.py, model_runner.py, ols_models.py, panel_models.py, model_recommender.py, discovery_*.py
│   │   ├── api/               # datasets.py, analyses.py, projects.py
│   │   ├── core/              # config.py, errors.py, logging.py
│   │   ├── models/            # 向后兼容的重导出 shim
│   │   ├── storage/           # database.py, models.py, repositories.py（SQLite + 内存缓存）
│   │   ├── schemas/           # dataset.py, modeling.py, project.py
...
│   │   ├── datasets/[datasetId]/discover/   # 探索性关系发现
│   │   ├── analyses/[analysisId]/           # 结果仪表板
│   │   ├── projects/                        # 项目列表、创建、详情
│   │   └── projects/[projectId]/            # 项目工作区、数据集、时间线
│   ├── components/
│   │   ├── modeling/                        # 变量选择、变换面板、模型配置
│   │   ├── projects/                        # 项目卡片、表单、概览、时间线、归档表
...
**后端**（可选，以下为默认值——详见 `backend/.env.example`）：
```
...
- 单用户本地 SQLite 存储，暂不支持多用户或云同步
- 无身份验证或多用户隔离
- 不支持任意公式输入（变量通过 UI 选择）
- PDF 导出需要原生 Pango/Cairo 库——DOCX 和 LaTeX 导出可作为替代
...
| Phase 5 | 自然语言研究规划、吸收变量透明化 | ✅ 已完成 |
| Phase 6 | 约束探索性关系发现 | ✅ 已完成 |
| Phase 7 | 持久化研究工作区和可复现项目存储 | ✅ 已完成 |
| Phase 8 | 出版级报告生成和高级学术导出 | ✅ 已完成 |
| Phase 9 | 高级计量模型和因果识别工作流 | 未来 |
