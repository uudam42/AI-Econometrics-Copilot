...
| 功能 | 说明 |
|---|---|
| 快速分析 | 四阶段引导式工作流：上传 → 自动规划 → 确认 → 通俗结果 |
| 双语界面 | 一键切换 中文 / English，选择跨会话保留 |
| Windows 桌面应用 | 独立 Tauri 安装程序 — 无需 Docker、Python、Node.js 或浏览器 |
...
│   └── requirements.txt
├── frontend/
│   ├── app/                # Next.js 页面（含 /quick-analyze 和索引页）
│   ├── components/         # UI 组件（建模、结果、项目、引导）
│   ├── lib/                # API 客户端、i18n（中/英）、工具函数
│   ├── types/              # TypeScript 接口
│   ├── Dockerfile
│   └── package.json
├── desktop/
...
- 提供 **简体中文** 和 **English** 两套词典
- 语言选择跨会话保留（localStorage）
- 系统语言为中文的浏览器自动默认显示中文
- 网页版和 Windows 桌面版均可使用

实现方式：客户端 React context（[`frontend/lib/i18n.tsx`](frontend/lib/i18n.tsx)），与 Tauri 桌面构建所需的静态导出完全兼容。
